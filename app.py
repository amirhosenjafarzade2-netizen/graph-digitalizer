...
<body>
  ...
  <script type="module">
    ...
    const ui = {
      ...
      setAxesBtn: $('set-axes'), resetAxisPointsBtn: $('reset-axis-points'),
      axisInputs: $('axis-inputs'), axisInstruction: $('axis-instruction'),
      calibrateBtn: $('calibrate'), resetCalibrationBtn: $('reset-calibration'),
      toggleGridBtn: $('toggle-grid'), toggleLogXBtn: $('toggle-log-x'), toggleLogYBtn: $('toggle-log-y'),
      addPointBtn: $('add-point'), adjustPointBtn: $('adjust-point'), deletePointBtn: $('delete-point'),
      highlightLineBtn: $('highlight-line'), highlightControls: $('highlight-controls'),
      nPointsInput: $('n-points'), deleteHighlightBtn: $('delete-highlight'),
      clearPointsBtn: $('clear-points'), sortPointsBtn: $('sort-points'),
      newLineBtn: $('new-line'), renameLineBtn: $('rename-line'), lineSelect: $('line-select'),
      importJsonBtn: $('import-json'), importJsonInput: $('import-json-input'),
      exportJsonBtn: $('export-json'), exportCsvBtn: $('export-csv'), exportXlsxBtn: $('export-xlsx'),
      clearSessionBtn: $('clear-session'), undoBtn: $('undo'), redoBtn: $('redo'),
      previewTable: $('preview-table'), statusBar: $('status-bar'), magnifierZoom: $('magnifier-zoom'),
      autoCenterChk: $('auto-center'), colorDetectCtrls: $('color-detection-controls'),
      bgColorInput: $('bg-color'), lineColorInput: $('line-color'),
      toleranceInput: $('color-tolerance'), radiusInput: $('search-radius')
    };

    // ---------- Helpers ----------
    const showModal = msg => {
      $('modal-content').innerHTML = `<p>${msg}</p><button onclick="document.getElementById('modal').style.display='none'">OK</button>`;
    ...
    ui.imageUpload.onchange = e => {
      ...
      reader.onload = ev => {
        ...
        state.img.onerror = () => { showModal('Failed to load image.'); showSpinner(false); };
      };
    ...
    const draw = () => {
      ...
      state.lines.forEach((line,i)=>{
        const col = ['#1f77b4','#ff7f0e','#2ca02c','#d62728','#9467bd'][i%5];
        line.points.forEach(p=>drawPoint(p.x,p.y,col,4));
        if (line.points.length>1) drawLine(line.points,col);
      });
    ...
    const drawPoint = (x,y,col,r) => {
      ...
      ui.ctx.strokeStyle='white'; ui.ctx.lineWidth=1; ui.ctx.stroke();
    };
    const drawLine = (pts,col,w=1) => {
      if (pts.length<2) return;
      ui.ctx.strokeStyle=col; ui.ctx.lineWidth=w;
      ui.ctx.beginPath(); ui.ctx.moveTo(pts[0].x,pts[0].y);
      pts.forEach(p=>ui.ctx.lineTo(p.x,p.y));
      ui.ctx.stroke();
    };
    const drawGrid = () => {
      ui.ctx.save(); ui.ctx.translate(state.panX,state.panY); ui.ctx.scale(state.zoom,state.zoom);
      ui.ctx.strokeStyle='#0004'; ui.ctx.lineWidth=1/state.zoom;
    ...
    const calibrate = () => {
      ...
      if (state.axisPoints.length<needed) return showModal('Not enough axis points.');
      ...
      if ([x1,x2,y1,y2].some(v=>isNaN(v))) return showModal('Invalid axis values.');
      ...
      updateAxisInstruction(); updateButtonStates(); draw(); saveState();
    };
    ...
    ui.canvas.addEventListener('mousedown', e=>{
      ...
      if (state.mode==='axes'){
        state.axisPoints.push({x,y});
        updateAxisInstruction(); draw(); saveSession(); return;
      }
    ...
    $('sort-points').onclick=()=>{
      const l=state.lines[state.currentLineIndex];
      l.sorted=!l.sorted;
      $('sort-points').classList.toggle('sort-active',l.sorted);
    ...
    $('new-line').onclick=()=>{
      ...
      if(name){
        ...
        updateLineSelect();updatePreview();draw();saveSession();
      }
    };
    $('rename-line').onclick=()=>{
      const l=state.lines[state.currentLineIndex];
      const name=prompt('Rename:',l.name);
      if(name){l.name=name;updateLineSelect();updatePreview();draw();saveSession();}
    };
    ...
    $('clear-session').onclick=()=>{
      if(confirm('Clear everything?')){localStorage.removeItem('digitizerState');location.reload();}
    };
    $('undo').onclick=undo;
    $('redo').onclick=redo;
    ...
    const setMode=m=>{
      state.mode=m;
      updateButtonStates();
      ui.highlightControls.style.display=m==='highlight'?'block':'none';
      ui.axisInputs.style.display=(m==='axes' && state.axisPoints.length>0 && !state.isCalibrated)?'block':'none';
      updateAxisInstruction();draw();saveSession();
    };
    ...
    const updateLineSelect=()=>{
      const sel=$('line-select'); sel.innerHTML='';
      state.lines.forEach((l,i)=>{const o=new Option(l.name,i);if(i===state.currentLineIndex)o.selected=true;sel.add(o);});
    };
    const updatePreview=()=>{
      const line=state.lines[state.currentLineIndex];
      const tbl=$('preview-table'); tbl.innerHTML='';
    ...
    const restore=s=>{
      ...
      updateLineSelect();updatePreview();updateButtonStates();updateAxisInstruction();draw();
    };
    ...
    window.addEventListener('resize',debounce(()=>{
      if(state.img.src){
        const ratio=state.img.naturalHeight/state.img.naturalWidth;
        ui.canvas.width=Math.min(state.img.naturalWidth,window.innerWidth*0.75);
        ui.canvas.height=ui.canvas.width*ratio;
        draw();
      }
    },200));
    ...
    // ---------- Initial UI ----------
    updateLineSelect(); updatePreview(); updateButtonStates(); updateAxisInstruction();
  </script>
</body>
</html>
"""
