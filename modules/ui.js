...
import { exportJson, exportCsv, exportXlsx } from './io.js';
import { saveSession } from './session.js';
import { showModal, showSpinner } from './utils.js';
...
export function bindButtons() {
    ...
    u.resetAxisPointsBtn.onclick = () => { state.axisPoints=[]; updateAxisInstruction(); draw(); saveSession(); };
    u.calibrateBtn.onclick = calibrate;
    u.resetCalibrationBtn.onclick = resetCalibration;
    ...
    u.deleteHighlightBtn.onclick = deleteHighlight;
    u.clearPointsBtn.onclick = clearCurrentLine;
    u.sortPointsBtn.onclick = sortCurrentLine;
    ...
    u.bgColorInput.oninput = () => { state.bgColor = hexToRgb(u.bgColorInput.value); saveSession(); };
    u.lineColorInput.oninput = () => { state.lineColor = hexToRgb(u.lineColorInput.value); saveSession(); };
    u.toleranceInput.oninput = () => { state.colorTolerance = parseInt(u.toleranceInput.value); saveSession(); };
    u.radiusInput.oninput = () => { state.searchRadius = parseInt(u.radiusInput.value); saveSession(); };
}
...
export function updateButtonStates() {
    ...
    const canEdit = calibrated && state.mode!=='axes';
    ...
    // enable/disable based on calibration
    ...
    u.renameLineBtn.disabled = !calibrated;
}
...
export function updatePreview() {
    ...
    const sorted = line.sorted ? [...line.points].sort((a,b)=>a.dataX-b.dataX)
...
export function updateAxisInstruction() {
    ...
    if (state.axisPoints.length < needed) {
        const label = document.getElementById('shared-origin').checked && state.axisPoints.length===0 ?
                      'Shared Origin (X1/Y1)' : ['X1','X2','Y1','Y2'][state.axisPoints.length];
        instr.textContent = `Click point for ${label} on the chart.`;
    } else {
